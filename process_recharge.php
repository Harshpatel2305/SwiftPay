...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $queryPlanId = "SELECT id FROM dth_plans WHERE operator_id = '$operatorId'";
    ...
    if ($resultPlanId && mysqli_num_rows($resultPlanId) > 0) {
        ...
        // Example: Insert data into the dth_transactions table with "PENDING" status
        $insertTransactionQuery = "INSERT INTO dth_transactions (user_id, mobile_number, operator_id, plan_id, amount, validity, description, approval_status, transaction_time) 
            VALUES (?, ?, ?, ?, ?, ?, ?, 'PENDING', NOW())";
